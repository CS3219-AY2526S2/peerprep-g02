...
export const registerSocketHandlers = (io: Server) => {
    io.on("connection", (socket: Socket) => {
        ...
        socket.on("join_queue", socketWrapper(socket, "join_queue", async (req: any) => {
            const matchDetails = MatchDetailsSchema.safeParse(req);

            if (!matchDetails.success) {
                socketLogger.warn(`Invalid join_queue request from user ${userId}: ${matchDetails.error}`);
        ...
        socket.on("cancel_queue", socketWrapper(socket, "cancel_queue", async () => {
        ...
        socket.on("disconnect", socketWrapper(socket, "disconnect", async () => {
