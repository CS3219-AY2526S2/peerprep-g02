import { BookOpen, Brain, CheckCircle, Code, Loader2, Sparkles, WifiOff, X } from "lucide-react";
...
export default function MatchSearchingView({
    ...
    return (
        <CardContent className="p-8 sm:p-12 bg-gradient-to-br from-indigo-900 via-purple-900 to-violet-900 text-white flex flex-col items-center justify-center min-h-[500px]">
            {/* Pulsing Radar / Status Animation */}
            <div className="relative flex items-center justify-center mb-10">
                {!isConnected ? (
                    ...
                    <>
                        <div
                            className="absolute inset-0 rounded-full bg-emerald-500/30 animate-pulse"
                            style={{ animationDuration: "2s" }}
                        ></div>
                        <div
                            className="absolute inset-2 rounded-full bg-emerald-400/40 animate-ping"
                            style={{ animationDuration: "2s" }}
                        ></div>
                        <div className="relative z-10 bg-white/10 p-5 rounded-full backdrop-blur-md border border-emerald-300/50 shadow-[0_0_30px_rgba(52,211,113,0.3)]">
                            <CheckCircle className="size-12 text-emerald-300 animate-in zoom-in duration-300" />
                        </div>
                    </>
                ) : (
                    <>
            ...
            <div className="text-center mb-8">
                {!isConnected ? (
                    ...
                    <>
                        <h3 className="text-2xl font-bold tracking-tight mb-2 flex items-center justify-center gap-2 text-emerald-300">
                            <Sparkles className="size-5" />
                            Match Found!
                        </h3>
                        <p className="text-emerald-100/80 max-w-[280px] mx-auto">
            ...
            <div className="w-full bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 mb-10 space-y-5 shadow-inner transition-opacity duration-300">
                {/* Topics */}
            ...
            {isPreparing ? (
                <Button
                    disabled
                    className="h-14 w-full rounded-xl bg-white/10 text-white/50 text-lg font-semibold border border-white/5 cursor-not-allowed"
                >
