...
export class UserScoreService {
    ...
    async applyScoreDeltas(updates: Array<{ clerkUserId: string; delta: number }>): Promise<
        Array<{
            clerkUserId: string;
            previousScore: number;
            newScore: number;
    ...
    > {
        ...
        if (!Array.isArray(scoreUpdates) || scoreUpdates.length !== updates.length) {
            throw new ServiceError(424, "User service returned an invalid score delta response.");
        }
    ...
    private async fetchUserScore<T>(url: string, init: RequestInit, action: string): Promise<T> {
