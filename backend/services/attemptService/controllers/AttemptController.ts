...
export class AttemptController {
  ...
  async create(req: Request, res: Response): Promise<Response | void> {
    ...
    if (
      typeof body?.userAId !== "string" ||
      typeof body?.userBId !== "string"
    ) {
  ...
  async listAttemptsForCurrentUser(
    _req: Request,
    res: Response,
  ): Promise<Response | void> {
    ...
    if (!clerkUserId) {
      return res
        .status(500)
        .json({ error: "Authenticated user context is missing." });
    }

    try {
      const response =
        await this.attemptService.listAttemptHistory(clerkUserId);
      return res.status(200).json(response);
    } catch (error) {
