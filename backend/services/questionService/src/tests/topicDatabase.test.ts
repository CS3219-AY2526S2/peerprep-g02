...
describe("Topics Service", () => {
  ...
  it("AddTopic should return true on success", async () => {
    ...
    (pool.query as any).mockResolvedValue({ rows: [{ tid: "5752a2a8-d4a6-4cc9-8fcf-bb4dfe3a0542" }, { tid: "5752a2a8-d4a6-4cc9-8fcf-bb4dfe3a0543" }] });
  ...
  it("EditTopic should return true on success", async () => {
    ...
    expect(pool.query).toHaveBeenCalledWith("UPDATE topics SET topic = $2 WHERE tid = $1 RETURNING tid", [
      "5752a2a8-d4a6-4cc9-8fcf-bb4dfe3a0542",
      "Physics",
    ]);
    expect(pool.query).toHaveBeenCalledWith("UPDATE topics SET topic = $2 WHERE tid = $1 RETURNING tid", [
      "5752a2a8-d4a6-4cc9-8fcf-bb4dfe3a0543",
      "Biology",
    ]);
  });
